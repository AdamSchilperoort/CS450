...
int main(int argc, char *argv[])
{
        ...
        *count = N;
        ...
        calcDistance<<<totalBlocks,1024>>>(dev_pointData, dev_count, dev_epsilon);
...
__global__ void calcDistance(struct pointData * data, unsigned int * count, double * epsilon)
{
        ...
        int i;
        double epsilonSquared = (*epsilon) * (*epsilon); //compute epsilonSquared once

        for(i=tid+1; i<N; i++)
        {
                if ( fabs( (data[tid].x - data[i].x ) * (data[tid].x - data[i].x) )
                   + fabs( (data[tid].y - data[i].y ) * (data[tid].y - data[i].y) )
                   <=  epsilonSquared )
                {
